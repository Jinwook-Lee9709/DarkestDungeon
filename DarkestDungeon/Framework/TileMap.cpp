...
{
	Set({ 8, 1 }, { 720.f, 720.f });
	texture = &GET_TEX(spriteSheetId);
	SetOrigin(Origins::ML);
	SetScale({ 1.f, 1.f });
	SetPosition( { 0.f, FRAMEWORK.GetWindowSizeF().y * 0.35f });
}
...
void TileMap::Set(const sf::Vector2i& count, const sf::Vector2f& size)
{
	...
	for (int i = 0; i < count.y; ++i)
	{
		...
		{
			int texIndex = Utils::RandomRange(1, 5);
			if ( j == 0 || j == count.x - 1)
			{
				texIndex = 0;
			}
			...
			for (int k = 0; k < 4; ++k)
			{
				...
				va[vertexIndex].texCoords.x += (texIndex % 3) * 720.f;
				va[vertexIndex].texCoords.y += (texIndex / 3) * 720.f;
			}
		}
	}
}
